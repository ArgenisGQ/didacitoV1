...
4. **Interfaces Frontend Premium y Glassmorphism (Efecto Cristal)**
   ...
   * **Gobernanza IT de Elite (`AdminSettings.tsx`):** Panel exclusivo para `SUPER_ADMIN` con sub-tabs responsivas de cristal, control de switches de políticas de accesos, editor dinámico de tags de columnas CSV requeridas y terminal SMTP interactivo.
    * **Gestión de Identidades Eficiente (`UserManagement.tsx`):** Barra superior de búsqueda con **Debounce de 300 ms** para aliviar la carga de peticiones al backend y selectores avanzados por rol y estado MFA. Adicionalmente, incluye un sistema dinámico de advertencias en color amarillo para detectar materias en la **Carga Académica** que carecen de programa sinóptico registrado o activo en el sistema, adaptando el modal detallado para alertar de forma explícita qué materia específica requiere carga de programa.
...
5. **Arquitectura Relacional Avanzada (Many-to-Many):** Asignación granular de cargas horarias donde un mismo docente puede dictar materias distintas en periodos distintos sin requerir cuentas duplicadas, manteniendo siempre la trazabilidad histórica de auditoría. Además, integra la validación inteligente de disponibilidad de programas sinópticos sobre la carga asignada, alertando visualmente con badges y notificaciones en amarillo si existen cursos pendientes de carga de syllabus.
...
11. **Visualización y Flujo del Docente (Correcciones):**
    * **Caja de Alerta en Edición (Wizard):** Al editar una planificación observada, se renderiza una alerta naranja con un icono de advertencia (`AlertTriangle`) en la parte superior del wizard que muestra de forma permanente y clara el feedback del coordinador en todos los pasos de la edición.
    * **Widget Colapsable e Interactivo:** En el panel de control del docente, la tarjeta **"Mis Planes Observados"** agrupa los planes devueltos en una lista limpia. Al seleccionar un plan (o hacer clic en **"Ver Detalle"**), el contenedor se expande revelando el mensaje del coordinador y el botón de acción rápida para corregirlo.
    * **Vista Previa Inline:** En el listado general de planes del docente se muestra un extracto rápido de la observación bajo el código de la asignatura para una rápida identificación.
12. **Actualizaciones y Sincronización en Tiempo Real (WebSockets):**
    * **Habilitación Global:** Se removió la restricción que impedía la conexión por WebSockets a docentes y coordinadores, permitiendo la comunicación en caliente para todos los roles del sistema.
    * **Refresco Automatizado (React Query):** Cuando el servidor WebSocket difunde un evento de cambio (`ANALYTICS_UPDATE`), el frontend intercepta el mensaje e invalida los queries en caché (`plans`, `analytics` y `academicLoad`). Esto provoca una recarga asíncrona de datos en segundo plano que actualiza la UI al instante sin necesidad de recargar la página.
    * **Lógica de Eliminación:** Se incorporó el broadcast de WebSockets en el endpoint `delete_plan` para asegurar que las eliminaciones notifiquen a todos los clientes activos.
